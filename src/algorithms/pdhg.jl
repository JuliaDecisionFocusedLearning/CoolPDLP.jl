...
function kkt_errors(state::PDHGState, sad::SaddlePointProblem{T, V}) where {T, V}
    ...
    primal = norm(ifelse.(ineq_cons, positive_part.(q - K * x), q - K * x))
    ...
    return KKTErrors(;
        primal, dual, gap, primal_scale, dual_scale, gap_scale, rel_max, weighted_agg
    )
end

function prepare_check!(state::PDHGState, sad::SaddlePointProblem, params::PDHGParameters)
    (; starting_time) = state
...
function get_results(state::PDHGState, sad::SaddlePointProblem)
    (; x, y) = state
    (; preconditioner) = sad
