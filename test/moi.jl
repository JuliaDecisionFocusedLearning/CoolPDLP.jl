...
@testset "Options survive repeated optimize!" begin
    model = JuMP.Model(CoolPDLP.Optimizer)
    JuMP.set_silent(model)
    JuMP.set_attribute(model, "matrix_type", CoolPDLP.GPUSparseMatrixCSR)
    JuMP.set_attribute(model, "backend", JLBackend())
    JuMP.set_attribute(model, "float_type", Float32)
    JuMP.set_attribute(model, "algorithm", CoolPDLP.PDLP)
    ...
    JuMP.optimize!(model)
    @test JuMP.termination_status(model) == MOI.OPTIMAL
    obj1 = JuMP.objective_value(model)

    # a second solve must reuse the same options instead of silently reverting to the defaults
    # popped out by the first call (see issue #106)
    JuMP.optimize!(model)
    @test JuMP.termination_status(model) == MOI.OPTIMAL
    @test JuMP.objective_value(model) ≈ obj1 atol = 1.0e-2

    @test JuMP.get_attribute(model, "matrix_type") == CoolPDLP.GPUSparseMatrixCSR
    @test JuMP.get_attribute(model, "float_type") == Float32
    @test JuMP.get_attribute(model, "algorithm") == CoolPDLP.PDLP
end
