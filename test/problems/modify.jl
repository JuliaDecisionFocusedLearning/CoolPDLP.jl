...
@testset "Set types" begin
    ...
    @test milp_f32 isa LinearProgram{Float32, Vector{Float32}, SparseMatrixCSC{Float32, Int}}
    milp_i32 = CoolPDLP.set_indtype(Int32, milp)
    @test milp_i32 isa LinearProgram{Float64, Vector{Float64}, SparseMatrixCSC{Float64, Int32}}
    milp_dense = CoolPDLP.set_matrix_type(Matrix, milp)
    @test milp_dense isa LinearProgram{Float64, Vector{Float64}, Matrix{Float64}}
...
@testset "Change backend" begin
    ...
    @test milp_flexible isa LinearProgram{Float64, Vector{Float64}, GPUSparseMatrixCSR{Float64, Int, Vector{Float64}, Vector{Int}}, Vector{Bool}}
    milp_gpu = adapt(JLBackend(), milp_flexible)
    @test milp_gpu isa LinearProgram{Float64, JLVector{Float64}, GPUSparseMatrixCSR{Float64, Int, JLVector{Float64}, JLVector{Int}}, JLVector{Bool}}
